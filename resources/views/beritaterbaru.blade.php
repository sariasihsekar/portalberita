...
<head>
    ...
    <title>Daftar Berita Terbaru</title>
...
<body>
    ...
    <a href="{{route('buatberitaterbaru')}}" class="btn btn-success btn-sm mb-3" >Buat Berita Baru</a>
        ...
        <tbody>
            @foreach($beritaTerbaru as $baru)
                <tr>
                    ...
                    <td>
                            <div>
                                <span class="short-content">{{ Str::limit($baru->konten, 20) }}...</span>
                                <span class="full-content">{{ $baru->konten }}</span>
                                <button class="btn btn-link p-0" onclick="toggleContent(this)">Lihat Selengkapnya</button>
                            </div>
                    </td>
                    <td>
                        <a href="{{ route('editberitaterbaru', $baru->id) }}" class="btn btn-success btn-sm">Edit</a>
                        <form action="{{ route('beritaterbaru.destroy',[ 'id' => $baru->id]) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
